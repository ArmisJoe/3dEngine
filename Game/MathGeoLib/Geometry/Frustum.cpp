...
bool Frustum::Contains(const Polyhedron &polyhedron) const
{
 ...
 CollisionType Frustum::ContainsBox(const AABB & aabb) const
{
		float3 vCorner[8];
		int iTotalIn = 0;
		aabb.GetCornerPoints(vCorner);

		//aabb->GetVertices(vCorner); // get the corners of the box into the vCorner array
									 // test all 8 corners against the 6 sides
									 // if all points are behind 1 specific plane, we are out
									 // if we are in with all points, then we are fully in
		Plane m_plane[6];
		GetPlanes(m_plane);
		//returns true if the point is in the plane or outside
		for (int p = 0; p < 6; ++p) {
			int iInCount = 8;
			int iPtIn = 1;
			for (int i = 0; i < 8; ++i) {
